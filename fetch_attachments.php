...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $applications_sql = "SELECT COUNT(*) as count FROM attachment_applications WHERE education_qualifications = '{$row['description']}'";
        ...
        if ($applications_count >= $row['attachment_limit']) {
            echo "<td style='border: 1px solid black; text-align: left; padding: 8px;'>Limit Reached</td>";
        } else {
            echo "<td style='border: 1px solid black; text-align: left; padding: 8px;'><a href='attachee_application.php?attachment_id=" . $row['id'] . "'>Apply</a></td>"; // Link to application form with attachment ID
        }
        echo "</tr>";
    }
    echo "</table>";
