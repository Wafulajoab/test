    ...
    <div id="page-content-wrapper">
        <nav class="navbar navbar-expand-lg navbar-light bg-transparent py-4 px-4">
            <div class="d-flex align-items-center">
                ...
                <h2 class="fs-2 m-0">Manage Jobs</h2>
            </div>
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            echo "<tr><th style='background-color: #f2f2f2; border: 1px solid black; text-align: left; padding: 8px;'>Title</th><th style='background-color: #f2f2f2; border: 1px solid black; text-align: left; padding: 8px;'>job_limit</th><th style='background-color: #f2f2f2; border: 1px solid black; text-align: left; padding: 8px;'>Description</th><th style='background-color: #f2f2f2; border: 1px solid black; text-align: left; padding: 8px;'>Requirements</th><th style='background-color: #f2f2f2; border: 1px solid black; text-align: left; padding: 8px;'>Application Deadline</th><th style='background-color: #f2f2f2; border: 1px solid black; text-align: left; padding: 8px;'>Action</th></tr>";

            // Fetch and display data row by row
            while ($row = mysqli_fetch_assoc($result)) {
                echo "<tr>";
                echo "<td style='border: 1px solid black; text-align: left; padding: 8px;'>" . (isset($row['title']) ? $row['title'] : 'N/A') . "</td>";
                echo "<td style='border: 1px solid black; text-align: left; padding: 8px;'>" . (isset($row['job_limit']) ? $row['job_limit'] : 'N/A') . "</td>";
                echo "<td style='border: 1px solid black; text-align: left; padding: 8px;'>" . (isset($row['description']) ? $row['description'] : 'N/A') . "</td>";
                echo "<td style='border: 1px solid black; text-align: left; padding: 8px;'>" . (isset($row['requirements']) ? $row['requirements'] : 'N/A') . "</td>";
                echo "<td style='border: 1px solid black; text-align: left; padding: 8px;'>" . (isset($row['application_deadline']) ? $row['application_deadline'] : 'N/A') . "</td>";
                echo "<td style='border: 1px solid black; text-align: left; padding: 8px;'><a href='edit_job.php?id=" . $row['id'] . "' style='text-decoration: none; color: blue;'>Edit</a> | <a href='delete_job.php?id=" . $row['id'] . "' onclick=\"return confirm('Are you sure you want to delete this job?')\" style='text-decoration: none; color: red;'>Delete</a></td>";
                echo "</tr>";
            }
            echo "</table>";
            echo "<br>";
            echo "<button style='background-color: #4CAF50; color: white; padding: 12px 20px; border: none; border-radius: 4px; cursor: pointer; font-size: 16px; margin-bottom: 20px;'><a href='jobs.php' style='text-decoration: none; color: white;'>Add New Job</a></button><br><br>";
        } else {
            echo "No jobs found.";
        }
